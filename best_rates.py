...
BR_Columns = ["radio","timestamp",
    "best_rates","macaddr","maxtp0",
    "maxtp1","maxtp2","maxtp3",
    "maxprob","J","K",
    "L","M","N",
    "O",'P','Q','R','S']
best_rates = pd.read_csv ("dataset/24.csv",low_memory=False,sep=";",names=BR_Columns)
to_drop_rates = ["J","K","L","M","N",
"O",'P','Q','R','S']
...
best_rates=best_rates.head(1000000)
best_rates = best_rates.iloc[700000:,:]

#best rates
best_rates=best_rates.drop(to_drop_rates, axis=1)
best_rates=best_rates[best_rates['best_rates'].str.contains('best_rates').fillna(False)]
best_rates=best_rates[best_rates['macaddr'].str.contains('bc:a8:a6:9c:6f:53').fillna(False)]
# print(best_rates)
best_rates=time_index(best_rates)
...
best_rates=best_rates.replace({"maxtp0": dict})
best_rates=best_rates.replace({"maxtp1": dict})
...
dummy,=plt.plot([best_rates['timestamp'].iat[0]]*len(ytick),ytick)
dummy.remove()
ax.scatter(best_rates["timestamp"],best_rates["maxtp0"],lw=2,marker="s", color='black',alpha=0.2)
# ax.scatter(best_rates["timestamp"],best_rates["maxtp1"],lw=2,marker="s", color='yellow',alpha=0.2)

plt.title("First Best-Rate map")
plt.xlabel("Time",fontsize=9)
plt.ylabel("MCSIndex",fontsize=9)
plt.xticks(fontsize=8, rotation=35,ha="right" )
plt.yticks(fontsize=10)
plt.savefig(f"plot/Bestplots/best_rates-bc:a8:a6:9c:6f:53-maxtp0.png",format='png',dpi=500, bbox_inches='tight')
# plt.show()
