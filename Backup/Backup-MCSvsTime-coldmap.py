...
for t1 in (["3T","1T","50s","1s","500ms","100ms","50ms"]):
    for mac,macdata in time_rolling.groupby(["macaddr"],sort=False):
        ...
        for time_rolling_subset in macdata.rolling(t1):
            ...
            fail_probability_cur = fail_probability["fail_probability"].astype(float)
            fprobability.append(fail_probability_cur)

    ### mcs + airtime
            # fail_probability['mcs']=fail_probability['rates']
            # # print(fail_probability.dtypes)
            # index=fail_probability.replace({"rates": dict})
            # index['rates']=index['rates'].apply(lambda x: int(x, 16))
            # index.sort_values(by=['rates'], inplace=True)
            # index['mcsair'] = fail_probability["mcs"].astype(str) +"-"+  index['rates'].astype(str)
            # index =index['mcsair']
    ...
    ### airtime
            ...
            # index['rates']=index['rates'].apply(lambda x: int(x, 16))
            # index.sort_values(by=['rates'], inplace=True)
            # # index['rates'] =index['rates'].astype(int)
            # index =index ['rates']
            # airtime.append(index)
...
# # ### mcs int 64
            fail_probability['rates']=fail_probability['rates'].apply(lambda x: int(x, 16))
            index=fail_probability['rates']
            airtime.append(index)
        ...
        for ii, (xe,ye) in enumerate(zip(Time,airtime)):
            plt.scatter([xe]*len(ye),ye, c=fprobability[ii],cmap='Blues',s=60,vmin=0,vmax=1,marker="x" )

        plt.xticks(rotation=-35,fontsize=18 , ha='left')
        plt.xlabel('Time',fontsize=22)
        plt.yticks(fontsize=19)
        plt.ylabel('Mcs rate - airtime',fontsize=22)
        plt.title(f"Client: {mac} -Fail probability map" ,fontsize=24 )
        plt.colorbar(matplotlib.cm.ScalarMappable(cmap='Blues',norm=plt.Normalize(vmin=0,vmax=1)))
        plt.savefig(f"plot/FPA/FP-{mac}-Rolling on{t1}-set2.svg",format='svg', bbox_inches='tight')
        plt.clf()
