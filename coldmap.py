...
for mac,macdata in time_conv.groupby(["macaddr"],sort=False):
    ...
    fail["group"]  = fail.group.apply(to_int)
    fail["offset"] = fail.offset.apply(to_int)
    fail = fail.replace("", 0)
    ...
    ColdMap = pd.pivot_table(fail , index='group', columns='offset' , values='failed_counts')
    ...
    ax = sns.heatmap(ColdMap_data,cmap="ocean_r" , norm=LogNorm(), annot_kws={"size":3},square=True,linewidths=.08,linecolor="turquoise")
    ...
    plt.savefig(f"plot/{label_str}-coldmap-{mac}-22-{index_range_str}.png" ,dpi=300)
    plt.close(fig)
