...
from process import attempt,time_conv,attempt_check
from cleaning import to_int,dataset,index_range_str,label_str
pd.options.mode.use_inf_as_na = True

attempt_check=attempt_check.fillna(0)
...
for mac,macdata in time_conv.groupby(["macaddr"],sort=False):
    ...
    attempt_check["group"]  = attempt_check.group.apply(to_int)
    attempt_check["offset"] = attempt_check.offset.apply(to_int)
    ...
    AttemptMap = pd.pivot_table(attempt_check , index='group', columns='offset' , values='Fullattempt_count')
    AttemptMap.fillna(0, inplace=True)
    AttemptMap_data = np.zeros((42,10))
    for idx,row in AttemptMap.iterrows():
            for offs in row.index:
                AttemptMap_data[idx][offs] = row[offs]
    plt.figure(figsize=(17, 15))
    ax = sns.heatmap(AttemptMap_data,cmap="Wistia" , norm=LogNorm(),annot_kws={"size":3},square=True,linestyle="dotted", linewidths=.08,linecolor="thistle")
    ...
    plt.savefig(f"plot/{label_str}-attemptmap-{mac}-22-{index_range_str}.png" ,dpi=300)
    plt.close(fig)
