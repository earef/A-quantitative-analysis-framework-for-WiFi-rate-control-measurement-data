...
### GI
        ...
        colors_list = plt.cm.PuRd(np.linspace(0.3, 1, 256))
        custom_cmap = colors.LinearSegmentedColormap.from_list('custom_PuRd', colors_list)
        ...
        # plt.title(f"Station: {mac} -Success probability map" ,fontsize=16 )
        plt.colorbar(matplotlib.cm.ScalarMappable(cmap=custom_cmap,norm=plt.Normalize(vmin=0,vmax=1)))
        plt.savefig(f"plot/{label_str}-MCS.vs.Time-heatprobabilitymap-{mac}-22-{index_range_str}-{t1}.png",format='png', dpi=500,bbox_inches='tight')
        plt.clf()
